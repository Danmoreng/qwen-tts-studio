...
- **Voice Lab:** Morph or average compatible speaker embeddings, inspect the current mix, preview the result, and save reusable voices.
...
- **Norm preservation:** Optionally rescale the result to the weighted source L2 norm. This is a stability heuristic, not a guarantee of natural speech.
- **Latent fingerprint:** Compare compact summaries for voice A, voice B, and the current output. Colour and edge markers show block polarity, while contrast-enhanced intensity shows relative RMS magnitude. Norm matching remains available as a compact toggle.
...
An embedding is a learned 1024- or 2048-value latent vector, not a waveform, spectrum, or 2D frequency curve. Its coordinate indices do not correspond to Hertz, time, pitch, gender, accent, or emotion. Compare results by ear.
